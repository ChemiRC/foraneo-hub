  ...
  return (
    <section className="pb-10 space-y-8 font-sans">
      ...
      <header className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <p className="text-slate-400 text-sm font-semibold">Modelo Transaccional Activo</p>
          </div>
        </div>
        <div className="flex items-center gap-2 bg-slate-900 text-white px-5 py-3 rounded-2xl shadow-lg shadow-slate-900/20">
          <span className="text-lg font-bold">◆</span>
          <div>
            <p className="text-[11px] font-black text-slate-300 uppercase tracking-widest">Comisión Foráneo Hub</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
        <MetricCard label="Venta Bruta (sesión)" value={`$${dailyStats.total.toFixed(2)}`} sub={`${dailyStats.count} transacciones`} />
        <MetricCard label="Comisión Retenida (6%)" value={`−$${dailyStats.fee.toFixed(2)}`} sub="Tarifa de plataforma" />
        <MetricCard label="Ingreso Neto a Transferir" value={`$${dailyStats.net.toFixed(2)} MXN`} sub="Liquidación T+1" accent wide />
      </div>

      {/* Model explanation */}
      <div className="bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 flex items-center gap-6 flex-wrap">
        <span className="text-2xl font-bold">ℹ</span>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-slate-700">Modelo de comisión</p>
          <p className="text-xs text-slate-400 font-medium mt-0.5">
            Foráneo Hub retiene <strong>6%</strong> por transacción. El <strong>94%</strong> se deposita en ciclo T+1.
          </p>
        </div>
        <div className="flex items-center gap-3 text-sm font-black shrink-0">
          <span className="bg-white border border-slate-200 rounded-xl px-4 py-2 text-slate-700">100%</span>
          <span className="text-slate-400">-</span>
          <span className="bg-red-50 border border-red-200 rounded-xl px-4 py-2 text-red-600">6%</span>
          <span className="text-slate-400">=</span>
          <span className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-2 text-emerald-700">94%</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-1 bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-6 flex-1 flex flex-col bg-slate-50">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-5 shadow-sm">
              <h4 className="text-[9px] font-black uppercase tracking-[0.15em] text-slate-400 border-b border-slate-100 pb-2 mb-3">Operación</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-slate-500 font-medium"><span>Monto cobrado</span><span className="font-bold text-slate-800">${ticketAmount || '0.00'}</span></div>
                <div className="flex justify-between text-slate-500 font-medium"><span>Comisión 6%</span><span className="font-bold text-red-500">−${fee}</span></div>
              </div>
              <div className="border-t border-slate-100 mt-3 pt-3 flex justify-between items-center">
                <span className="text-sm font-black text-emerald-700">Neto</span>
                <span className="text-2xl font-black text-emerald-600">${net}</span>
              </div>
            </div>

            <button onClick={handleCharge} disabled={isProcessing || !ticketAmount} className="w-full mt-auto bg-slate-900 hover:bg-indigo-600 text-white font-black py-4 rounded-2xl transition-all shadow-lg shadow-slate-900/20 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2.5 text-sm">
              {isProcessing ? <><span className="animate-spin">⚙</span> Procesando…</> : <><span>■</span> Procesar Pago</>}
            </button>
          </div>
        </div>
        ...
        <div className="xl:col-span-2 bg-white rounded-[2rem] border border-slate-100 shadow-sm flex flex-col overflow-hidden">
          <div className="px-8 py-5 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-black text-slate-800 text-lg">Actividad de la Cuenta</h3>
